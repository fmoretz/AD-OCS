...
for i in range(len(t_span)):  
    F_W[i] = water_percentage*Q_in[i]*rho_water/18*1000         # [mol/d]    - Water Flow 
    F_M[i] = q_M[i]*V_liq[i]                                    # [mol/d]   - Methane Flow 
    F_C[i] = q_C[i]*V_liq[i]                                    # [mol/d]   - CO2 Flow
    F_S[i] = q_S[i]*V_liq[i]                                    # [mol/d]   - Sulfur Flow

    F_i[i] = np.array([F_M[i], F_C[i], F_S[i], F_W[i]])         # [mol/d] - Outlet Molar Flow - In of the flash
