...
d_end   = 60       # [h] - End time
hours   = 1/10      # [h] - Discretization time
X_CRIT = 50 # ppm
...
for ind in range(len(t_span)):
    ...
    while ind < (len(t_span)-len(t_cstr)):                
        ...
        df_iter = pd.DataFrame({'t_in': t_span[ind],'t': (t_span[ind]+tau_headspace[ind]),'F_CH4': N_V[ind,0], 'F_CO2': N_V[ind,1], 'F_H2S': N['H2S'][-1], 'F_H2O': N['H2O'][-1], 'F_O2': N['O2'][-1], 'r_avg': (r.mean()*24)}, index=[ind])
        digester_out = pd.concat([digester_out,df_iter])
        break

x_H2S = np.zeros(len(t_span))

digester_out_opt = pd.DataFrame()
headspace_dict_opt = {}
N_in_O2 = 0.5*N_V[:,2]
i = 0
for ind in range(len(t_span)):
    ...
    while ind < (len(t_span)-len(t_cstr)):   
        ...
        N, r = headspace_dynamics_discr(N_V[ind:(ind+len(t_cstr)),:], N_in_O2[ind:(ind+len(t_cstr))], P_dig, T, V_gas, t_cstr, k_SOB, RC, alfa, beta)
        headspace_dict_opt[ind]= {'t_in': t_span[ind],'t_cstr': t_cstr, 'H2S': N['H2S'], 'H2O': N['H2O'], 'O2': N['O2'], 'SX': N['SX'], 'r_sob': r} # stores the results of the CSTR dynamics at each iteration, r [mg/L/h], N [mol/h]
        df_iter = pd.DataFrame({'t_in': t_span[ind],'t': (t_span[ind]+tau_headspace[ind]),'F_CH4': N_V[ind,0], 'F_CO2': N_V[ind,1], 'F_H2S': N['H2S'][-1], 'F_H2O': N['H2O'][-1], 'F_O2': N['O2'][-1], 'r_avg': (r.mean()*24)}, index=[ind])
        if ind not in  digester_out_opt.index:
            digester_out_opt = pd.concat([digester_out_opt,df_iter])
        else:
            digester_out_opt = pd.concat([digester_out_opt[:ind],df_iter])
        F_tot = digester_out_opt.iloc[ind]['F_CH4']+digester_out_opt.iloc[ind]['F_CO2']+digester_out_opt.iloc[ind]['F_H2S']+digester_out_opt.iloc[ind]['F_O2']+digester_out_opt.iloc[ind]['F_H2O']
        x_H2S[ind] = digester_out_opt.iloc[ind]['F_H2S']/F_tot*1e+6 #ppm
        
        if x_H2S[ind] > X_CRIT:
            print('CRITICITY: x_H2S = ', x_H2S[ind], 'at t = ', t_span[ind])
        else:
            control = 'OK'
            print('ok: x_H2S = ', x_H2S[ind], 'at t = ', t_span[ind])
        if control == 'OK':
            break
        else:            
            N_in_O2[ind:(ind+len(t_cstr))] = N_in_O2[ind:(ind+len(t_cstr))]*1.1
digester_out['efficiency'] = (1-digester_out['F_H2S'] / N_V[0:len(digester_out),2])*100
digester_out_opt['efficiency'] = (1-digester_out_opt['F_H2S'] / N_V[0:len(digester_out_opt),2])*100

# EFFECTS OF OXYGEN INHIBITION ON THE DIGESTER

x_V_O2 = digester_out['F_O2']/(digester_out['F_CH4']+digester_out['F_CO2']+digester_out['F_H2S']+digester_out['F_O2']+digester_out['F_H2O'])
...
digester_out['F_tot'] = digester_out['F_CH4']+digester_out['F_CO2']+digester_out['F_H2S']+digester_out['F_O2']+digester_out['F_H2O']
...
# Elaboration of the results: molar fractions, volumetric flows, etc.
digester_out_opt['F_tot'] = digester_out_opt['F_CH4']+digester_out_opt['F_CO2']+digester_out_opt['F_H2S']+digester_out_opt['F_O2']+digester_out_opt['F_H2O']

# Create columns in digester_out_opt accounting for molar fractions
digester_out_opt['x_CH4'] = digester_out_opt['F_CH4']/digester_out_opt['F_tot']
digester_out_opt['x_CO2'] = digester_out_opt['F_CO2']/digester_out_opt['F_tot']
digester_out_opt['x_H2S'] = digester_out_opt['F_H2S']/digester_out_opt['F_tot']
digester_out_opt['x_H2O'] = digester_out_opt['F_H2O']/digester_out_opt['F_tot']
digester_out_opt['x_O2']  = digester_out_opt['F_O2']/digester_out_opt['F_tot']

# Create columns in digester_out_opt for volumetric flowrates at given temperature and pressure

digester_out_opt['Q_CH4'] = digester_out_opt['F_CH4']*Rgas_m3_atm_K*(T_norm+273.15)/P_norm          # [m3/h] - Volumetric flowrate of CH4
digester_out_opt['Q_CO2'] = digester_out_opt['F_CO2']*Rgas_m3_atm_K*(T_norm+273.15)/P_norm          # [m3/h] - Volumetric flowrate of CO2
digester_out_opt['Q_H2S'] = digester_out_opt['F_H2S']*Rgas_m3_atm_K*(T_norm+273.15)/P_norm          # [m3/h] - Volumetric flowrate of H2S
digester_out_opt['Q_H2O'] = digester_out_opt['F_H2O']*Rgas_m3_atm_K*(T_norm+273.15)/P_norm          # [m3/h] - Volumetric flowrate of H2O
digester_out_opt['Q_O2']  = digester_out_opt['F_O2']*Rgas_m3_atm_K*(T_norm+273.15)/P_norm           # [m3/h] - Volumetric flowrate of O2
digester_out_opt['Q_total'] = digester_out_opt['Q_CH4']+digester_out_opt['Q_CO2']+digester_out_opt['Q_H2S']+digester_out_opt['Q_H2O']+digester_out_opt['Q_O2'] # [m3/h] - Total volumetric flowrate

digester_out_opt['x_v_CH4'] = digester_out_opt['Q_CH4']/digester_out_opt['Q_total']          # [-] - Vol. Fraction of CH4 in the total flowrate
digester_out_opt['x_v_CO2'] = digester_out_opt['Q_CO2']/digester_out_opt['Q_total']          # [-] - Vol. Fraction of CO2 in the total flowrate
digester_out_opt['x_v_H2S'] = digester_out_opt['Q_H2S']/digester_out_opt['Q_total']          # [-] - Vol. Fraction of H2S in the total flowrate
digester_out_opt['x_v_H2O'] = digester_out_opt['Q_H2O']/digester_out_opt['Q_total']          # [-] - Vol. Fraction of H2O in the total flowrate
digester_out_opt['x_v_O2']  = digester_out_opt['Q_O2']/digester_out_opt['Q_total']           # [-] - Vol. Fraction of O2 in the total flowrate

plt.figure()
plt.subplot(2,1,1)

plt.plot(digester_out['t'], X_CRIT*np.ones(len(digester_out['t'])), '--', color = 'red', label='limit')
...
plt.figure()
plt.subplot(3,1,1)
plt.title('Residual Oxygen')
plt.plot()
plt.plot(digester_out_opt['t'], digester_out_opt['F_O2'])
plt.ylabel('F_O2 [mol/h]')
plt.subplot(3,1,2)
...
# plt.plot(digester_out['t'], digester_out['x_v_H2O'], label='H2O')
