...
in order to reach the desired H2S concentration in the effluent.
 - V2: Limit to maximum influent flow rate
    - V2.2: Limit on maximum growth ratio and new variable for the setpoint (F_O2_SP)'''
...
for i in range(0, len(t)-1):#   
    ...
    while True: 
        ...
        if x_out['H2S'][i+1]*1e+6 > H2S_treshold: 
            ...
            F_O2_SP[i-1]    = min(F_in_max,F_O2_SP[i-1]*1.2) # Fixes the new setpoint by increasing it by 20%
            F_in['O2'][i-1] = max(F_O2_SP[i-1], F_in['O2'][i-2] + DELTA_F) # Increase O2 influent to have a larger concentration of O2 in the headspace at t=i -> r_SOB[i] can  be increased
            ...
            if F_in['O2'][i-1] >= F_in_max or F_in :
                print('Maximum O2 influent reached. \n Sorry not sorry, we move on.')
                break
        else:
            break
