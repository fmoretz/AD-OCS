...
digester_out_SS = headspace_dynamics_discr_SS(np.linspace(0,100,1000), N_V, N_in_O2, F_OUT_0, V_gas, tau_headspace, RC, alpha, beta, k_SOB, P_dig, T, T_norm, P_norm)
F_out_SS = {'H2S': digester_out_SS.iloc[-1]['F_H2S'],
            'CH4': digester_out_SS.iloc[-1]['F_CH4'],
            'CO2': digester_out_SS.iloc[-1]['F_CO2'],
            'O2' : digester_out_SS.iloc[-1]['F_O2'],
            'H2O': digester_out_SS.iloc[-1]['F_H2O'], 
            'SX' : digester_out_SS.iloc[-1]['n_SX']
            } # [mol/h] - Final flow, SX is [mol]

digester_out = headspace_dynamics_discr(t_span, N_V, N_in_O2, F_out_SS, V_gas, tau_headspace, RC, alpha, beta, k_SOB, P_dig, T, T_norm, P_norm)
digester_out.head()
print(digester_out)
plt.figure()
t_span = digester_out['t_out']
plt.subplot(2,1,1)
plt.plot(t_span, digester_out['x_CH4'], label = 'Methane')
plt.plot(t_span, digester_out['x_CO2'], label = 'Carbon Dioxide')
plt.grid(True); plt.xlabel('hours'); plt.ylabel('Molar Fraction [-]'); plt.legend()
plt.subplot(2,1,2)
plt.plot(t_span, digester_out['x_H2S_in']*1e+6, '--', label = 'x_H2S_in')
plt.plot(t_span, digester_out['x_H2S']*1e+6, label = 'x_H2S_out')
plt.grid(True); plt.xlabel('hours'); plt.ylabel('Molar Fraction [-]'); plt.legend()
plt.legend()
plt.figure()
t_span = digester_out['t_out']
plt.plot(t_span, digester_out['F_H2S_in'], label = 'H2S')
plt.plot(t_span, digester_out['F_O2_in'], label = 'O2')
plt.grid(True); plt.xlabel('hours'); plt.ylabel('Molar Flow [mol/h]'); plt.legend()
plt.show()
