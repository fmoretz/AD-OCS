...
def headspace_dynamics_discr(t, Nin, N_in_gas, F_out_0,*args):
  # Nin = [CH4, CO2, H2S, H2O]
  ...
  for i in range(0, len(t)-1):#   
      ...
      c_out['H2S'][i] = x_out['H2S'][i] * P/Rgas_L_atm_K/T  # mol/L
      c_out['O2'][i]  = x_out['O2'][i]  * P/Rgas_L_atm_K/T  # mol/L
  ...
  return F_out, F_in, r_SOB

def headspace_dynamics_discr_SS(t, Nin, N_in_gas, F_out_0,*args):
  # Nin = [CH4, CO2, H2S, H2O]
  Vgas, tau_head, R_C, alpha, beta, k, P, T, T_norm, P_norm = args
  T = T + 273.15
  ...
  F_in = {
      ...
      'O2':  N_in_gas['O2'][0]*np.ones(len(t)),
      'SX':  0.000*np.ones(len(t)),  
      'N2':  N_in_gas['N2'][0]*np.ones(len(t)),    
      'total': np.ones(len(t))
  }  
  for i in range(len(t)):
    F_in['total'][i] = sum(F_in[comp][i] for comp in F_in.keys() if comp != 'total')
