...
for j in range(len(t_span)):
    ...
    Ss_max[j] = gammaS_in[j]*1000/64*S2[j]                   #*y_influent[j,4]/y_influent[0,4]
    Xs_max[j] = Y_srb/(1-Y_srb)*Ss_max[j]     
    
    # Gompertz function for microbial population and dissolved sulfur
    if j > 1:
        mu_srb[j] = max(0.1*max(mu_srb[:j-1]),abs((- X2[0] + X2[j])/max(1e-16,t_span[j]-t_span[0])))      # [g/L/d]    - Gompertz parameter for SRB growth
    else:
