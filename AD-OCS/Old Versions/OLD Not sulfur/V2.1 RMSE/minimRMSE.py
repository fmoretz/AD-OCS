...
def rmse(x, reg_number, k_hyd, mu_1, mu_2) -> float:
    ...
    if reg_number == 4:
        y4exp = Dil*(S1_in - S1) + k_hyd*XT
        for i in range(0, len(XT)):
            y4exp[i] = (X_1[i]*(S1_in - S1[i]) + k_hyd*XT[i])
    ...
    if reg_number == 6:
        for i in range(0, len(Dil)):
            Loclist.append(qMeval(Dil[i], S1[i], S2[i], XT[i], S1_in, S2_in, k_hyd, x[0], x[1]))

        return np.sqrt(np.mean(q_M-Loclist)**2)

    if reg_number == 7:
        for i in range(0, len(Dil)):
            Loclist.append(qCeval(Dil[i], S1[i], XT[i], q_M[i], C[i], C_in, S1_in, k_hyd, x[0], x[1]))
