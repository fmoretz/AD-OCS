...
'''ADM1 simulation following the bsm2 approach. 
  Volumetric flow rate is assumed to be constant and defined from HRT as q_ad = V_ad/HRT
...
pH = 7.21285990436289 # initial pH
...
initflow = {'q_gas': [0], 'q_ch4': [0], 'q_co2' :[0], 'q_h2' :[0], 'p_gas' : [0], 'p_gas_h2' : [0], 'p_gas_ch4' : [0], 'p_gas_co2' : [0] , 'HRT': HRT, 'Qin': q_ad}
...
print('CHECK 2')
print(q_ad)
simulate_results.to_csv(name + "_dynamic_out.csv", index = False)
gasflow.to_csv(name+ "_gas_flows.csv", index = False) #m^3.d^-1
