...
'''Sequence of multiple ADM1 simulation following the bsm2 approach. 
  Volumetric flow rate is assumed at each iteration to be constant and defined from HRT as q_ad = V_ad/HRT
  Check the values of f_i_xc and k_hyd_i to ensure that the model is in the correct range of applicability'''
...
#folder =  'C:/Users/AD-OCS/ADM1_data/' # folder where the input output files will be saved
...
for i in range(len(HRT_vett)):
    ...
    for u in t[1:]:
      ...
      gascolumns = ['q_gas', 'q_ch4', 'q_co2', 'q_h2', 'p_gas', 'p_gas_h2', 'p_gas_ch4', 'p_gas_co2']
    ...
    if i == 0:      
      ...
      df_SS = df_SS.append(SS_values, ignore_index=True)
      df_SS_gas = df_SS_gas.append(SS_gas, ignore_index=True)
    print('ITERATION: ', i+1, 'HRT',HRT)
df_SS.set_index([HRT_vett])
df_SS.to_csv(name + "_SS_values" + ".csv")
df_SS_gas.to_csv(name + "_SS_gas" + ".csv")
