...
d_end = 40         # [d] - End time
hours = 2         # [h] - Discretization time
n_times = int((200)*24/hours) # Number of time steps
...
for i in range(len(t_span)):
    ...
    def Headspace_reactions(y):
        ...
        C_S  = y_S_loc*P_dig/(Rgas_L_atm_K*T)*1e+3 # [mmol/L] - Sulfur concentration in gas phase
        C_O2 = y_O_loc*P_dig/(Rgas_L_atm_K*T)*1e+3 # [mmol/L] - Oxygen concentration in gas phase

        n_S  = n_out[i]*y_S_loc
        n_O2 = n_out[i]*y_O_loc

        r_SOB = k_SOB*((C_S)**alfa)*(C_O2**beta)                          # [mmol/L/d] - Reaction rate SOB
        BM_S  = n_S_int[i] - n_S    + coeff_S*V_gas*r_SOB*1e+3            # [mmol/d]   - Sulfur Balance
        BM_O2 = n_O2_in    - n_O2   + coeff_O2*V_gas*r_SOB*1e+3           # [mmol/d]   - Oxygen Balance
        return [BM_S, BM_O2]
