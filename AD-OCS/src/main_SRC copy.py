...
YOUT = odeint(AD_OCS, y0, t_span_d, args=(alfa, mu_max, Ks, KI2, KH, Pt, kLa, D, k, kd, N_bac, N_S1, y0[2], t_span[0], y_in_0, T3.index.values, X2_pre, t_span))
...
def headspace_dynamics(x, t, N_in, N_in_O2, V_gas):
    ...
    n_S = max(0,x[2])                                                    # [mol/h] - Sulfur Flow
    n_W = x[3]                                                    # [mol/h] - Water Flow
    n_O = max(0,x[4])                                                  # [mol/h] - Oxygen Flow
    n_Sx = max(0,x[5])                                                   # [mol/h] - Sulfur solid
    ...
    r_SOB = k_SOB*(S_S**alfa)*(S_O**beta)                         # [1/m3/h] - Reaction rate
    dMdt = n_in_M - n_M
    dCdt = n_in_C - n_C
    dSdt = n_in_S - n_S - 2*r_SOB*V_gas/MW_S
    dWdt = n_in_W - n_W + 2*r_SOB*V_gas/18
    dOdt = n_in_O - n_O -   r_SOB*V_gas/16
    dSxdt =               2*r_SOB*V_gas/32
    print(t, r_SOB)
    return [dMdt, dCdt, dSdt, dWdt, dOdt, dSxdt]

N_in_O2 = 1*N_V[:,2]                                                        # [mol/h] - Oxygen Flow proportional to the sulfur flow
tau_headspace = ((V_reactor - V_liq[:]) + V_headspace)/Q_V[0,:].sum()       # [h] - Residence time of the headspace
t_span_headspace = np.linspace(0, tau_headspace[0], 10000)                  # [h] - Time vector for the headspace dynamics
...
plt.subplot(2,1,2)
plt.plot(t_span_headspace, 1-N_headspace[:,2]/N_V[0,2])
plt.show()
...
#     n_O2_in = 0.5*n_S_in            # [mol/h] - Influent Oxygen flowrate
